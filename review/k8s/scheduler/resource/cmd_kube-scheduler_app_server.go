...
func NewSchedulerCommand(registryOptions ...Option) *cobra.Command {
	...
	//设置cobra默认的command命令参数，如果用户有带参数则传入
	cmd := &cobra.Command{
		Use: "kube-scheduler",
...
//执行过程基本是做一些检查类的操作，比如打印日志、安装一些所需的
//环境组件之类的
...
func Run(ctx context.Context, cc *schedulerserverconfig.CompletedConfig, sched *scheduler.Scheduler) error {
	...
	//看一下是否在选举中，如果是的话返回自己isLeader的状态位
	//scheduler中由于实现了高可用，需要每个成员去抢占endpoint，
	//抢占得到则意味着是leader，并且每隔一段时间就会去renew一下，
	//以保持leader的地位。
	...
	//应该是将healthz健康检测handler添加到handler链上面了，
	//handler的调用应该是链式调用，但是为什么需要链式调用，需要后面研究一下
