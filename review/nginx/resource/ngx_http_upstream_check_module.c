...
ngx_http_upstream_check_init_shm(ngx_conf_t *cf, void *conf)
{
    ...
    //如果main config之中的peers数组成员数量不为空，则意味着是nginx本次是进行平滑重启。
    //平滑重启时，需要从原有的main config中读出一些参数进行加载。
    if (ucmcf->peers->peers.nelts > 0) {
        ...
        //在内存池中分配一块空间给shm name，固定长度为SHM_NAME_LEN，默认是256字节
        ...
        //如果原有的main config有规定shm size则使用原有的配置， 否则默认设置为1m
        shm_size = shm_size < ucmcf->check_shm_size ?
                              ucmcf->check_shm_size : shm_size;

        //在共享内存中按shm_size登记（此时不一定会分配）一块空间，命名为shm zone，并且以shm name为索引,
        //ngx_http_upstream_check_module为tag，ngx_shared_memory_add根据名称和tag去查找，如果已经存在
        //的话将会直接返回已经存在的zone地址
        ...
        //将shm 空间的data指向刚刚的内存池
        shm_zone->data = cf->pool;

        //将原有main config中的peers分配到上下文context中做为静态变量
        check_peers_ctx = ucmcf->peers;

        //至此zone分配空间结束，然后对zone进行初始化操作
    ...
    ngx_uint_t generation)
{
    ...
    //从内存池中分配一个空间给shm name，并让shm_name的data指针指向该内存池空间
    shm_name->data = ngx_palloc(pool, SHM_NAME_LEN);
    if (shm_name->data == NULL) {
        return NGX_ERROR;
    }

    //计算空间长度，并赋予shm_name->len
...
//tips 1:初始化shm空间，对于nginx来讲，一般都需要create完再进行init，
//create是为了先分配空间，而且往往会先根据之前的“经验”（old_cycle）先预分配空间，
//来避免后续空间不够而进一步扩容。
//tips 2:cycle在nginx中指的是一次启动周期，nginx使用cycle模型来实现平滑启动的功能，
//以达到热更新配置而不停止服务的功能。
static ngx_int_t
ngx_http_upstream_check_init_shm_zone(ngx_shm_zone_t *shm_zone, void *data)
{
    size_t                               size;
    //旧cycle的shm name，变量中以o开头的一般都默认是指old cycly的参数，后续不赘述
    ngx_str_t                            oshm_name;
    ngx_int_t                            rc;
